...
namespace laser_uav_lib
{
        ...
        : logger_(rclcpp::get_logger("NotchFilter"))
    {
        ...
        {
            RCLCPP_ERROR(logger_, "Parameters frequencies and bandwidths needs to have the same non-zero length!!");
        ...
        filter = std::make_unique<laser_uav_lib::IIRFilter>(a_std, b_std);

        RCLCPP_INFO(logger_, "Notch filter initialized!");
    }
