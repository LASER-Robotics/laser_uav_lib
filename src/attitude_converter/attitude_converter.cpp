...
namespace laser_uav_lib
{
    ...
    AttitudeConverter::AttitudeConverter(const laser_uav_lib::EulerAttitude &euler_attitude)
    {
    ...
    double AttitudeConverter::getYawRateIntrinsic(const double &heading_rate)
    {
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("AttitudeConverter"), "getYawRateIntrinsic(): \"projected_norm\" in denominator is almost zero!!!");
            throw MathErrorException();
        }

        double direction = std::copysign(1, orbital_velocity.dot(projected));

        double output_yaw_rate = direction * (orbital_velocity_norm / projected_norm);

        if (!std::isfinite(output_yaw_rate))
        {
            RCLCPP_ERROR(rclcpp::get_logger("AttitudeConverter"), "getYawRateIntrinsic() : NaN detected in variable \"output_yaw_rate\"!!!");
            throw MathErrorException();
        }
    ...
    double AttitudeConverter::getHeadingRate(const Vector3Converter &attitude_rate)
    {
        ...
        if (fabs(denom) <= 1e-5)
        {
            RCLCPP_ERROR(rclcpp::get_logger("AttitudeConverter"), "getHeadingRate(): denominator near zero!!!");
            throw MathErrorException();
        }
